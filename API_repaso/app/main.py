...
class Usuario(BaseModel):
    nombre: str = Field(..., min_length=2, max_length=50, example="Estrella Marco")
    email: str = Field(..., example="estrella@email.com")
...
class LibroCreate(BaseModel):
    nombre: str = Field(..., min_length=2, max_length=100, example="Crepusculo")
    autor: str = Field(..., min_length=2, max_length=100, example="Stephenie Meyer")
    anio: int = Field(..., example=2005)
    paginas: int = Field(..., gt=1, example=512)
...
async def devolver_libro(id: int):
    ...
    # 409 si el préstamo ya no existe (ya fue devuelto/eliminado)
